            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "error" in result:
                        ...
                        with col2:
                            xgb_score = ml_scores.get("xgb_score", 0)
                            st.metric("XGB Score", f"{xgb_score:.3f}")
                        with col3:
                            ensemble_score = ml_scores.get("ensemble_score", 0)
                            status = verification.get("status", classify_score(ensemble_score))
                            st.metric("Final Score", f"{ensemble_score:.3f}", 
                                     delta=status.upper())
    ...
    with st.spinner("Loading ML scores..."):
        try:
            ...
            fig = px.histogram(
                ...
                labels={"ensemble_score": "Fraud Score", "count": "Frequency"}
            )
            ...
            feature_importance = {
                "purchase_value": 0.35,
                "device_id_count": 0.25,
                "ip_address_count": 0.20,
                "time_to_purchase": 0.15,
                "age": 0.10,
                "source": 0.08,
                "browser": 0.05,
                "sex": 0.02
            ...
            st.info("""
            **The system uses XGBoost for fraud detection (71.41% recall).**
            
            - **XGBoost**: A gradient boosting framework that builds sequential models to detect fraud patterns
            - **Features**: Analyzes purchase value, device/IP usage patterns, timing, and user demographics
            - **Recall**: 71.41% - catches 71% of actual fraud cases
            - **Thresholds**: 
              - Pass: < 0.3 (Low risk)
              - Flag: 0.3 - 0.6 (Medium risk, manual review)
              - Deny: >= 0.6 (High risk, automatic rejection)
            
            The model was trained on historical transaction data with proper feature engineering including
            scaling (StandardScaler, MinMaxScaler, RobustScaler) and one-hot encoding for categorical variables.
            """)
